...
export default function Header(){
    ...
    return <>
        <nav className="flex items-center justify-between flex-wrap bg-indigo-600 p-6 mb-4">
        <a href="/shop" className="flex items-center flex-shrink-0 text-white mr-6 ml-20">
                ...
                {loginInfo && loginInfo.id !== null && loginInfo.length !== 0 && (
                    <>
                        <a href={`/favourites/${loginInfo.id}`}>
                            <button className="mr-6 inline-block text-sm px-4 py-2 leading-none border rounded text-white border-white hover:bg-white hover:border-indigo-600 hover:text-indigo-600 mt-4 lg:mt-0">
                                <span style={{ display: 'flex', alignItems: 'center' }}>
                                    <FaRegHeart style={{ fontSize: '1.5em', marginRight: '' }}/>
                                </span>
                            </button>
                        </a>
                        <a href={`/users/${loginInfo.id}`}>
                            <button className="mr-6 inline-block text-sm px-4 py-2 leading-none border rounded text-white border-white hover:bg-white hover:border-indigo-600 hover:text-indigo-600 mt-4 lg:mt-0">
                                <span style={{ display: 'flex', alignItems: 'center' }}>
                                    <VscAccount style={{ fontSize: '1.5em', marginRight: '7px' }} />
                                    See my profile
                                </span>
                            </button>
                        </a>
                        <a href="/shop/add">
                            <button className="mr-28 inline-block text-sm px-4 py-2 leading-none border rounded text-white border-white hover:bg-white hover:border-indigo-600 hover:text-indigo-600 mt-4 lg:mt-0">
                                <span style={{ display: 'flex', alignItems: 'center' }}>
                                    <VscAdd style={{ fontSize: '1.5em', marginRight: '7px' }}/>
                                    Add a product
                                </span>
                            </button>
                        </a>
